 ...
 *
 * والشرط هو البروتوكول نفسه لا استثناء localhost. كان يُعفى localhost وحده،
 * فأي عنوان `http` آخر يأخذ `Secure`. والمتصفّح يهمل كوكي `Secure` وصلته على
 * `http`، فلا تُحفظ الجلسة ولا يظهر خطأ: يسجّل المستخدم دخوله فيعود إلى صفحة
 * الدخول بلا سبب ظاهر. وهذا ما كان سيحدث بمجرد نقل العنوان من localhost إلى
 * عنوان الشبكة، وهو نقلٌ لازم ليصل الجوال.
 *
 * و`Secure` على `https` يبقى كما هو، فلا شيء يُفقد في الإنتاج.
 */
